...
| Pagination | MCP list resources use `page`, `page_size` (capped at 50), and `cursor` where the API supports it; the HTTP client sends `size` for list GETs. Unparameterized fetches return the first page only — no silent multi-page aggregation. Claude pages explicitly when it needs more. |
...
**Pagination.** List resources use `page`, `page_size` (MCP and resource template), and `cursor` where supported. The client maps `page_size` to the Archivist list query param `size` (server-side cap: 50). An unparameterized list fetch returns the first page only — there is no silent multi-page aggregation on the server side. Claude pages explicitly when it needs more, which keeps the per-call cost legible and cacheable.
...
4. **✓** Pagination passthrough. List resources accept `page`, `page_size`, and `cursor` and build list GETs with `size` (not `page_size` on the wire), capped at 50. Document the params in each list resource's docstring. Verify in Claude Desktop that fetching page 2 works.
   - **Tests:** `httpx` mock assertions for outgoing query (expect `size`, not `page_size`); `page_size` above 50 is clamped; `cursor` round-trips; end-to-end manual verification in Claude Desktop noted in PR.
